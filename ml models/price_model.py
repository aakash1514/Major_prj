...
class PriceModel:
    """
    A class to load and use the trained price prediction model.
    Loads the model from models/price_predictor_xgb.pkl using joblib.
    """
    
    def __init__(self, model_path='models/price_predictor_xgb.pkl'):
        """
        Initialize the PriceModel by loading the trained model.
        
        Args:
            model_path (str): Path to the saved model file. Defaults to 'models/price_predictor_xgb.pkl'
        ...
        try:
            ...
            print(f"✓ Price model loaded successfully from {self.model_path}")
        except Exception as e:
            raise Exception(f"Error loading model from {self.model_path}: {str(e)}")
    
    def predict(self, crop_features):
        """
        Predict the market price for a given crop.
        ...
        Returns:
            float: Predicted market price for the crop
        ...
        try:
            ...
            # Return the predicted price (handle both single and multiple predictions)
            ...
            raise ValueError(f"Error during price prediction: {str(e)}")
    ...
    def predict_batch(self, crop_features_list):
        ...
        try:
            ...
            raise ValueError(f"Error during batch price prediction: {str(e)}")
...
if __name__ == "__main__":
    try:
        ...
        sample_crop = [[1, 100, 25, 60]]  # Example: [crop_type, quantity, temp, humidity]
        
        predicted_price = price_model.predict(sample_crop)
