...
import { authenticateToken, authenticateUserOrCheckoutToken, requireRole } from '../middleware/authMiddleware.js';
...
router.post('/:id/payment/checkout-token', authenticateToken, requireRole('buyer'), ordersController.createCheckoutTokenForOrder);
router.get('/:id/payment/checkout', ordersController.renderRazorpayCheckout);
router.post('/:id/payment/verify', authenticateUserOrCheckoutToken, requireRole('buyer'), ordersController.verifyRazorpayPayment);
...
router.put('/:id/status', authenticateToken, requireRole('admin', 'agent'), ordersController.updateOrderStatus);
